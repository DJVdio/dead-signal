...
- **Sim 测不了什么，用它的数字前必须知道**：`Duel` 是 **1v1、无距离、无走位、无多目标**的引擎级对决，也**不建模噪音与弹药**。所以霰弹枪的短射程代价（射程 90 + 全表最重距离衰减）和"一发散射同时打中多只丧尸"的清群优势，Sim **一个都测不出**（它白送贴脸）；枪械胜率（如步枪 vs 长剑 93.5%）只能读作**「无限弹药 / 贴脸 / 单挑」下的杀伤力天花板**，不是"能打多久"——枪的真实战力由弹药供给决定，而供给在 loot/配方数值里，不在 Sim 里。空间侧机制（噪音/门/多目标/潜行）只能实机校准。
...
- 数值表（xlsx）是给用户看的：**中文名主键、人话说明，不出现类名/英文 id/引擎术语**（内部 id 收进置灰"勿改"列）。
