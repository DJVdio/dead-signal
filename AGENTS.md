...
- **百分比加成一律乘算，禁止加算**（用户拍板的通则）：缺两指的山姆是 `0.86 × 1.03 = 0.8858`，不是加算的 `0.89`——加算会让**没有手的人**（操作能力 0）凭空获得 3% 操作能力。新写加成时一律连乘。历史三处加算残留（诺蒂读速 / 南丁格尔感染率 / 手术恢复效率点数池）**已于 2026-07-17 全部整改为乘算**，各有护栏测试钉死（分别 ×1.875 / ×0.765 / 1.10×1.09=1.199；第三条经用户裁定"算百分比加成"）——**别再重复挖这三条**。
...
- 🔴 **胜率不是成本 —— 做经济/掉落分析前必读**（用户原话：「一趟白捡……什么意思，**战斗难道不是成本吗**」）。胜率只说"你能不能站着走出这一场"，它**一个字都没说**你为此付了什么：打光的弹药、要愈合 **7 昼夜**的骨折（占床、不能干活、不能站岗）、**永久**断掉的手、感染竞速、手术耗材与失败率、不可复活的死人。
  - **绝不要用「胜率 × 敌人数」估收益**。反例（**数字全部出自 `docs/research/2026-07-14-combat-cost.md`，Sim `cost` 模式，2026-07-17 于 commit `bd867a8` 重跑落盘（harness 确定性已验：固定种子、连跑两次 MD5 相同）；那份报告是机器生成的真源，以它为准、别信这里的抄写**）：**持棍棒劫掠者胜率 75.5%，但 76.2% 的胜场留下骨折**（每处卧床 7 昼夜、占床、不能干活、不能站岗）；**持破甲锤劫掠者胜率只有 38.1%，且 87.8% 骨折**。反过来**丧尸 1v1 胜率 100%**，却仍有 25.8% 的胜场毫发无伤——**另外 74% 的场次你是带着伤口回来的，每一道都要一台手术。** 按胜率排和按成本排，顺序完全不同。
  - 🔴 **围攻不能拿 1v1 胜率去想（平方律，`docs/research/2026-07-14-lanchester.md`，2026-07-17 于 commit `bd867a8` 重跑落盘·以报告为准）**：丧尸 **1 只 100% → 2 只 84.5% → 3 只 22.0% → 4 只 1.6% → 5 只 0%**（跌破 50% 的临界点约 **2.6 只**）。**是断崖，不是斜坡。**「打赢 8 个劫掠者白捡 8 把武器」这个场景根本不存在。
    ⚠️ 该报告的前提：**`Arena` 不建模碰撞体积/攻击距离/走位 ⇒ 围攻无名额上限**（8 只能同时贴脸砍一个人）⇒ 这组数是**围攻代价的上界（最坏情况）**，真实关卡里同时够得着一个人的丧尸数受几何限制。
  - 正确框架：**「这场仗要拿多少伤病和人命去换」**。数在 `docs/research/2026-07-14-combat-cost.md`，harness = `src/DeadSignal.Sim/CombatCostCalibration.cs`。
- **Sim 测不了什么，用它的数字前必须知道**：Sim 有**两套 harness，局限不同，别混着读**——`Duel` 是 **1v1、无多目标**的引擎级对决；`Arena` 是**真群战**（`Arena.Run(teamA, teamB, seeds)`，3v8 是逐个真结算的 ⇒ **平方律在 Arena 输出里已内生，不该再手动叠一次**）。两者**共同**的局限：**无距离、无走位、不建模噪音与弹药**。所以霰弹枪的短射程代价（射程 90 + 全表最重距离衰减）和"一发散射同时打中多只丧尸"的清群优势，Sim **一个都测不出**（它白送贴脸）；枪械胜率（如步枪 vs 长剑手·中甲 98.9%，出自 `docs/research/2026-07-13-weapon-recalib.md`——**该报告本轮未复跑，此处仅作举例，别当基线引用**）只能读作**「无限弹药 / 贴脸 / 单挑」下的杀伤力天花板**，不是"能打多久"——枪的真实战力由弹药供给决定，而供给在 loot/配方数值里，不在 Sim 里。空间侧机制（噪音/门/多目标/潜行）只能实机校准。
