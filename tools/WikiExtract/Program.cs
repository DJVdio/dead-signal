...
internal static class Program
{
    ...
    private static Category Weapons()
    {
        var cols = new List<Col>
        {
            ...
            new("kind", "种类", "chip", ReadOnly: true, Hint: "由武器自身属性推导（远程/弹药/伤害类型），不是可改字段"),
            new("damageType", "伤害类型", "chip"),
            new("damageMin", "伤害下限", "number"),
            new("damageMax", "伤害上限", "number"),
            new("penetration", "穿透", "number", Hint: "0~1，0.25 = 无视 25% 护甲"),
            ...
            new("ammoPerAttack", "每次攻击耗弹", "number", ReadOnly: true, Hint: "由连发数派生"),
            ...
            new("falloffFloor", "最远处伤害系数", "number", Hint: "超过射程末端时剩几成伤害"),
            ...
            new("stockPenetration", "枪托近战 穿透", "number"),
            new("structureFactor", "砸墙系数", "number"),
    ...
    private static Category Armor()
    {
        ...
        {
            new("name", "名称", Primary: true),
            new("layer", "甲层", "chip", Hint: "同一部位可叠穿：贴身 → 外套 → 护甲板"),
            new("equipSlot", "穿戴槽", "chip", ReadOnly: true, Hint: "成对装备（手套/鞋）一件占一槽，两件才护全"),
            new("sharpDefense", "锐防", "number"),
            new("bluntDefense", "钝防", "number"),
            new("weight", "重量(公斤)", "number"),
            new("covers", "覆盖部位", Hint: "多个部位用「、」隔开"),
            new("description", "说明", "longtext"),
            new("_id", "内部 id", Internal: true),
            new("_anchor", "代码位置", Internal: true),
        };
        ...
        {
            if (member.StartsWith("Dog", StringComparison.Ordinal)) continue; // 狗装备单列一个分区
            rows.Add(ArmorRow(member, a));
        }
        return new Category("armor", "护甲服装",
            "src/DeadSignal.Combat/ArmorTable.cs",
            "人穿的衣服与护甲。护甲值按部位覆盖生效——没覆盖到的部位一点也不挡。"
            + "「腐皮」是丧尸天生的一层，不是能穿的衣服。",
            cols, rows);
    }

    private static Dictionary<string, object?> ArmorRow(string member, ArmorLayer a) => new()
    {
        ["name"] = a.Name,
        ["layer"] = DisplayNames.Of(a.Slot),
        ["equipSlot"] = EquipSlotLabel(a.Name),
        ["sharpDefense"] = a.SharpDefense,
        ["bluntDefense"] = a.BluntDefense,
        ["weight"] = a.Weight,
        ["covers"] = a.CoversParts is null ? "" : string.Join('、', a.CoversParts),
        ["description"] = a.Description,
        ["_id"] = member,
        ["_anchor"] = $"src/DeadSignal.Combat/ArmorTable.cs :: ArmorTable.{member}()",
    };

    private static string EquipSlotLabel(string armorName)
    {
        ApparelCatalog.ApparelDef? def = ApparelCatalog.Get(armorName);
        if (def is null) return "";
        return string.Join('、', def.Slots.Select(DisplayNames.Of).Distinct());
    }
    ...
    private static Category Ammo()
    {
        var cols = new List<Col>
        {
            ...
            new("damageMult", "伤害倍率", "number", Hint: "箭专用：反过来改写弓/弩的属性"),
            new("penetrationMult", "破甲倍率", "number"),
            new("rangeMult", "射程倍率", "number"),
            new("cooldownMult", "冷却倍率", "number", Hint: "大于 1 = 出手更慢"),
            new("spreadMult", "散布倍率", "number", Hint: "大于 1 = 更不准"),
    ...
    private static Category Medical()
    {
        var cols = new List<Col>
        {
            ...
            new("efficacy", "治疗效率", "number", Hint: "抗生素 1.00 是满效；草药是它的零头"),
            new("worsenMult", "恶化减缓", "number", Hint: "越小越能拖住病情；空 = 不影响恶化"),
            new("weight", "单位重量(公斤)", "number"),
            new("description", "说明", "longtext"),
            new("_id", "内部 id", Internal: true),
            new("_anchor", "代码位置", Internal: true),
    ...
    private static Category Lights()
    {
        var cols = new List<Col>
        {
            ...
            new("intensity", "亮度", "number", Hint: "0~1。照亮自己，也照亮别人眼里的你"),
    ...
    private static Category Books()
    {
        var cols = new List<Col>
        {
            ...
            new("unlocks", "效果", Hint: "读完这本书带来什么：解锁的配方，或给的被动加成"),
            new("medicalPoints", "手术加成点数", "number", Hint: "读过的医疗书给施术者加手术点；不对玩家显示具体数字"),
        ...
        foreach (BookData b in BookLibrary.All())
        {
            ...
            rows.Add(new Dictionary<string, object?>
            {
                ...
                ["unlocks"] = RecipesUnlockedBy(b.Id),
                ["medicalPoints"] = pts > 0 ? pts : (object?)null,
                ["body"] = b.Body,
                ["_id"] = b.Id,
    ...
    private static Category WeaponMods()
    {
        ...
        {
            new("name", "名称", Primary: true),
            new("weaponClass", "能装在什么武器上", "chip"),
            new("fitsOn", "可装于", ReadOnly: true, Hint: "引擎按「武器大类」限制，这里把该大类的武器逐个列出来——加了新武器它会自己出现"),
            new("part", "占用部位", "chip", Hint: "一个部位只能装一件；不同部位可以同时装"),
            new("stats", "数值改动", ReadOnly: true, Hint: "结构化数据，改它要连带改代码——想调数值请直接跟 agent 说"),
        ...
        foreach (WeaponMod m in WeaponModCatalog.All())
        {
            ...
            {
                ["name"] = m.Name,
                ["weaponClass"] = DisplayNames.Of(m.RequiredClass),
                ["fitsOn"] = FitsOn(m.RequiredClass),
    ...
    /// <summary>
    /// 这项改装到底装得到哪几把武器上。引擎的约束只有「武器大类」（<c>WeaponMods.ClassOf</c>：远程=枪械、
    /// 近战锐器=刃、近战钝器=钝），但用户想看的是**具体哪几把枪**，所以这里把大类展开成武器名。
    /// 天生武器（爪击/撕咬/拳脚）和弓弩不在其列：前者不是装备，后者不吃枪械改装（<c>ClassOf</c> 会把弓算成枪械，
    /// 但弓没有枪管/枪托可拆——故按 <see cref="Weapon.AmmoKey"/> 把吃箭的排除掉）。
    /// </summary>
    private static string FitsOn(WeaponClass cls)
    {
        var names = new List<string>();
        foreach ((string member, Weapon w) in CatalogOf<Weapon>(typeof(WeaponTable)))
        {
            if (member is "ZombieClaw" or "DogBite" or "Fists") continue;  // 天生武器，不是能改的东西
            if (w.AmmoKey == AmmoKeys.Arrow) continue;                    // 弓弩没有枪管枪托可拆
            if (DeadSignal.Godot.WeaponMods.ClassOf(w) == cls) names.Add(w.Name);
        }
    ...
    private static Category Food()
    {
        var cols = new List<Col>
        {
            ...
            new("portions", "几个够做一份饭", "number", ReadOnly: true, Hint: "按当前的一份 16 点算出来的，改「热量点」它就跟着变"),
            new("description", "说明", "longtext"),
            new("_id", "内部 id", Internal: true),
            new("_anchor", "代码位置", Internal: true),
        };
    ...
    private static Category CookingRules()
    {
        var cols = new List<Col>
        {
            ...
            new("value", "数值", "number"),
            new("unit", "单位", "chip", ReadOnly: true),
            new("note", "说明", "longtext"),
            new("_id", "内部 id", Internal: true),
            new("_anchor", "代码位置", Internal: true),
        };
    ...
    private static Category Furniture()
    {
        var cols = new List<Col>
        {
            ...
            new("buildMinutes", "建造工时(游戏分钟)", "number"),
            new("salvage", "拆了能还回多少", ReadOnly: true, Hint: "通用还一半；木材例外——一半变木料、一半变废木料"),
            new("_id", "内部 id", Internal: true),
            new("_anchor", "代码位置", Internal: true),
        };
