...
/// </para>
/// <para>
/// 🔴 依据 <c>sim-lanchester</c> 实测：<b>2 只丧尸围攻胜率 16.6%、3 只 0.8%</b> ⇒ 围攻是**断崖**。
/// 所以"最多同时被 1 只感知"不是风味参数，是这一关能不能兑现用户设计的**生死线**。
/// </para>
/// </summary>
public class SewerTests
{
    ...
    public void 硬不变量_通道内任何位置能感知到你的丧尸都不超过一只()
    {
        ...
        Assert.True(
            worst.Count == 0,
            $"下水道有 {worst.Count} 个位置会被 2 只以上丧尸同时感知（围攻＝胜率 16.6%，这一关的口径是「基本没有危险」）。" +
