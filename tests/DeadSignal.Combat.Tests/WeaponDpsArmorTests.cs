...
public class WeaponDpsArmorTests
{
    ...
    //   | 皮夹克+布衣     | 2 | 34.8% → **26.4%**        | 11.4 → **15.4** |
    //   | 板甲+粗布外套+布衣 | 3 | 69.2% → **65.9%**     | 7.1  → **7.5**  |
    //   方向正确：多层单位不再白吃"层数减伤" ⇒ 挡下率下降、到肉伤害上升。
    ...
    [Fact]
    public void 交叉验证_皮夹克挡下约26点4百分比()
    {
        ...
        Assert.InRange(blocked, 0.255, 0.275);    // [T59] 重测：26.4%
    }
    ...
    public void 交叉验证_板甲挡下约65点9百分比()
    {
        ...
        Assert.InRange(blocked, 0.65, 0.67);      // [T59] 重测：65.9%
    }
