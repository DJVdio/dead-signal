...
public class ArcheryTests
{
    ...
    //
    // 用户原话（旧）：「弓箭射程+10%，锥形角-10%，攻速+2%」；[T68] 射程+10% 已换成弹道速度+20%（现已落地）⇒ 三项都**只作用于弓弩**（"弓箭"），
    // 且都是**射手本人读过书**才吃到（判据＝其 ReadBookSet，与回收率 25%→50% 同一条口径）。

    [Fact]
    public void 书_三项加成的方向与幅度_用户口径()
    {
        // 🔴 [T68] 用户把「射程 +10%」换成了「弹道速度 +20%」；飞速轴另测，射程加成保持中和为 ×1.0。
        Assert.Equal(1.0, Archery.BookRangeMult);           // [T68] 射程加成删除，改由 BookFlightSpeedMult ×1.20
