...
sb.AppendLine(CultureInfo.InvariantCulture,
    $"2. **切除偏高，需拍板【存疑】**：无甲切除率 狙击枪 {Get("狙击枪", "无甲").SeverRate:P1}、步枪 {Get("步枪", "无甲").SeverRate:P1}、重剑 {Get("重剑", "无甲").SeverRate:P1}——因规则\"单击≥部位MaxHP即切除\"叠加细部位低血量（眼3/手10/脚10），强武器打中小部位几乎必断。若不想角色被频繁肢解，需上调细部位 HP 或把切除门槛收紧（如仅四肢可断、或 dmg≥MaxHP×系数）。满甲后大幅回落（狙击 {Get("狙击枪", "板甲+皮甲+布衣").SeverRate:P1}）。");
sb.AppendLine(CultureInfo.InvariantCulture,
