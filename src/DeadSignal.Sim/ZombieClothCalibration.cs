...
/// 「丧尸穿生前的破衣服」前后对比校准。
/// <para>
/// 护甲值、武器区间、穿透均来自 Wiki 配置。逐层结算 atk ∈ [伤害下限,上限]、def ∈ [0, 护甲值×(1−穿透)]，atk &lt; def/2 才算挡下。
/// ⇒ <b>单层甲有可能挡下一击 ⟺ 护甲值×(1−穿透)/2 &gt; 武器伤害下限</b>。
/// </para>
...
public static class ZombieClothCalibration
{
    ...
    private static void Elites(StringBuilder sb)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("> 防暴警察丧尸穿**板甲**，躯干主要受护甲保护；能赢仍取决于命中未覆盖部位。");
        sb.AppendLine("> ——具体护甲数值与覆盖部位以 Wiki 配置表为准。");
        sb.AppendLine();
    }
    ...
    private static void Distribution(StringBuilder sb)
    {
        ...
        sb.AppendLine("  而非给防御值打折——具体防护值与覆盖部位以 Wiki 配置表为准。");
    }
}
