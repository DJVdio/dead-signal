...
public sealed class CombatEffectResolver
{
    ...
    public EffectOutcome Apply(Body body, Weapon weapon, CombatResult result, double concussionResistFactor = 1.0)
    {
        ...
        //    下表是**一次性的 A/B 实验存档**（`lanchester`，同一棵树、**同为 100/0.55 口径**，只切读法）：
        //    | N 只丧尸围攻 | 改前(T59) | **读法 A** | **读法 B（本实现）** |
        //    | 2 只 | 61.5% | **15.7%** 🔴 | **84.5%** |
        //    | 3 只 | 11.1% | **0.4%**  🔴 | **22.0%** |
        //    ⚠️ **三列的时效性不同，别一起读**：
        //      · 「改前」「读法 A」两列是**跑不出第二次的历史实验臂**（那两套代码已不存在）⇒ 永久存档、不会变。
        //      · 「读法 B」列 = **当前实现**，会随引擎/数值改动而漂 ⇒ **现值一律以
        //        `docs/research/2026-07-14-lanchester.md` 为准**（上表 84.5% / 22.0% ＝ 2026-07-17 全仓重跑值；
        //        此前这里写的 84.4% / 24.1% 抄自 `991b777`，那份报告已被证实是 born-stale 假重跑）。
        //    🔴 **别把「读法 A 的 15.7%」与 `BleedModel` 里 [T53] 的「16.6%」混为一谈** —— 两个数巧合都在 ~16%，
        //       但**前提完全不同**：15.7% 是**本表的读法 A 臂（100/0.55 口径）**；16.6% 是 **[T53] 的 70/1.5 热口径**
        //       （与三级流血无关，是更早的另一套配置）。**两条都是准确历史，别互相"修正"、别合并。**
        //    （读法 B 的权威标定值以 <c>BleedModel.SeverityRateOf</c> 长注为准，此处仅示意，勿另行维护第二份数）
        ...
        //    读法 B 不但不冲突，还**兑现了三级流血的设计初衷**：浅爪多半连血都挂不上、挂上也只是小流血（0.3），
        //    ⇒ **2 只丧尸从 61.5% 抬到 84.5%、3 只从 11.1% 抬到 22.0%**（BleedModel 标定值）—— 封顶真的打断了平方律。
