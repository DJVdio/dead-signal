...
public static class WeaponTable
{
    ...
    // ⚠️ 下面五条 summary 里的数字是 **2026-07-17 核对 weapons.json / wiki 武器表** 的当时值，
    //    而**数值由用户在 wiki 上随时手改**（wiki 是数值唯一设计源）。**真源永远是工厂取到的配置，不是这些字眼**。
    //    此前这批注释抄的是「批次18 / T21 手改」的历史值，用户后续又改了数次而注释没跟 ⇒ 全表对不上（已按实际值改写）。
    //    要看当前值：读 godot/data/config/weapons.json，别信注释。

    /// <summary>匕首：近战锐器，伤害 1~7、穿透 <b>7.5%</b>、冷却 1.7s（DPS 2.35）。全表最快的锐器。</summary>
    public static Weapon Dagger() => CombatCatalog.Weapon("dagger");

    /// <summary>短剑：近战锐器，伤害 3~8、穿透 12%、冷却 2.1s（DPS 2.62）。</summary>
    public static Weapon Shortsword() => CombatCatalog.Weapon("shortsword");

    /// <summary>刺剑：单手近战锐器，突刺路线——伤害区间窄、攻速略快。可双持。
    /// 伤害 2.5~6.5、穿透 <b>25%</b>（明显高于短剑 12%）、冷却 1.9s（DPS 2.37）。</summary>
    public static Weapon Rapier() => CombatCatalog.Weapon("rapier");

    /// <summary>长剑：近战锐器，伤害 3~15、穿透 <b>24%</b>、冷却 3.2s（DPS 2.81）。双手武器。</summary>
    public static Weapon Longsword() => CombatCatalog.Weapon("longsword");

    /// <summary>重剑：近战锐器，伤害 6~20、穿透 <b>40%</b>、冷却 4s（DPS 3.25）。双手武器。
    /// <b>全近战最高穿透</b>（破甲锤仅 15%）—— 但**远不及步枪的 70%**（旧注释「已与步枪持平」写于步枪 40% 的年代，已作废）。</summary>
    ...
    /// </para>
    /// <para>
    /// ✅ <b>「拳脚冷却 1.2→1.4」那条用户手改<u>已落地</u></b>（2026-07-17 核对 weapons.json：<c>fists</c> = 1~3 / 1.4s）
    /// ⇒ 拳脚 DPS <b>1.4286 ＜ 骨刀 1.50</b> ✓，本武器"比空手强一点"的定位现已真实成立
    /// （旧注释写的"待 <c>impl-unarmed</c> 落地"已作废）。
    /// 护栏仍钉在**匕首**（2.3529）—— 它跨两代数值未变，是本文件最稳的锚。
    /// </para>
    /// </summary>
    ...
    //
    // 钝器打**人**是弱的（棍棒垫底），这是**有意的代价**，不是待修的平衡问题；
    // ⚠️ 此处曾抄「棍棒打丧尸 47.8% / 尖头锤 75.0%」两个 Sim 胜率 —— **已删**：那是棍棒还是 4~7（DPS 2.04）
    //    时跑的，用户此后把它改到 4~8（DPS 2.22），数字 born-stale 且注释里的胜率没有门禁覆盖。
    //    要胜率去跑 Sim（`dotnet run --project src/DeadSignal.Sim`），别信注释里的裸数字。
    //    🔴 且**胜率不是成本**：它一个字都没说你为此付了什么（弹药/骨折 7 昼夜/永久断手/感染）。
    ...
    /// 现值 <b>4~8 / 2.7s ＝ DPS 2.22</b>（2026-07-17 核对 weapons.json；旧注释写的「4~7 ＝ 2.04」已作废）。
    /// 破甲为零 ⇒ 几乎打不动甲。这个压力是设计的一部分，<b>别当平衡问题"修"</b>。
    /// （比它低的只有骨刀 1.50／草叉 2.17／拳脚 1.43 —— 前两者是应急/农具，不是"武器路线"。）</para>
    ///
    /// <para>⚠ <b>它同时是枪托近战的锚点</b>：原则「任一枪托 DPS ＜ 棍棒」⇒ <b>改棍棒 = 改枪托的天花板</b>。
    /// 该锚点<b>目前是成立的</b>（枪托 1.72~1.80 ＜ 棍棒 2.22），由 <c>GunModBenchTests</c> 的正向硬护栏钉死
    ...
    /// <para><b>现值</b>（2026-07-17 核对 weapons.json，用户后续手改过多轮、皆拟定待调）：
    /// 伤害 <b>7.5~17.5</b>、穿透 <b>40%</b>、冷却 <b>4s</b>、射程 260 / 满伤 90 / 末端 0.45、枪托钝击 <b>3~6</b>。
    /// （旧注释抄的立项值「4~16 / 25% / 3.5s / 枪托 5~9」<b>全部作废</b>。）</para>
    /// <para><b>⚠ 它不是"穿甲专精"</b>：立项时的派单原话是"穿透比步枪还高 → 专克穿甲目标"，
    /// 但用户此后把<b>步枪穿透一路提到 70%</b> ⇒ 自制猎枪 40% <b>远低于</b>步枪。故
    ...
    //   一句话：**比拳头强，但不如一把真正的刀。**
    //
    // ══════ ✅ 锚点已修复：用户后来自己把枪托调下去了（2026-07-17 核对 weapons.json 实算）══════
    //
    // 上面那三个锚点数字（拳脚 2.08 / 匕首 2.86 / 棍棒 4.79）**全部作废**：T21/T29 用户在数值表上
    // **连续大幅下调近战**，一度出现「枪托反超棍棒」的失衡（曾记录为"锚点塌了、bug 复活"）。
    // **用户原话：「枪托先不管了，我后面自己调整。」——后来他真的调整了**：枪托整条窄带已被压到
    // 全部近战武器之下。现在的真实 DPS 序（引擎 WeaponDps 实算，值在 weapons.json）：
    //
    //   拳脚 1.43 ＜ 骨刀 1.50 ＜ 【枪托 1.72~1.80】 ＜ 短弓 1.88 ＜ 草叉 2.17 ＜ 棍棒 2.22 ＜ 匕首 2.35
    //            ＜ 刺剑 2.37 ＜ 短剑 2.62 ＜ 长剑 2.81 ＜ 尖头锤 2.86 ＜ 消防斧 3.02 ＜ 破甲锤 3.11 ＜ 重剑 3.25
    //   （枪托明细：手枪 1.72 ＜ 冲锋枪 1.74 ＜ 步枪/狙击 1.79 ＜ 自制猎枪/霰弹 1.80）
    //
    // ⇒ **T7 定稿的原则重新成立**：拳脚 ＜ 枪托 ＜ 一把真正的近战武器。**比拳头强，但不如一把刀。**
    ...
    //
    // 【穿透仍是枪托的第二道天花板】枪托穿透仅 0.02~0.03 ⇒ 对**披甲**目标极差；对**丧尸**
    //   （腐皮 3/3，几乎不挡）差别小 —— 但现在 DPS 本身已经压住了，不再依赖这条缓解因素。
    //
    ...
    /// <summary>冲锋枪：远程锐器，穿透 <b>27%</b>（2026-07-17 核对 weapons.json；旧注释写的 18% 已作废）。双手抵肩；枪托可贴脸钝击。
    ...
    /// <b>1 只 100% → 2 只 84.5% → 3 只 22.0% → 4 只 1.6% → 5 只 0%</b>。
    /// ⚠️ 本段两版旧注释都在这栽过：一版写「2 只从 82.6% 掉到 16.6%」（点名引用的报告里根本没这两个数），
    /// 一版抄 84.4%/24.1%（出自 <c>991b777</c> 的 born-stale 假重跑）—— 所以这里只留量级 + 时效，真数去看报告。
    ...
    /// <para><b>现值</b>（2026-07-17 核对 weapons.json）：伤害 <b>2~4</b>、穿透 <b>15%</b>、冷却 2.3s（=爪击同速）⇒ DPS 1.30。
    /// 旧注释写的「穿透 10%」「伤害压到 1-4」<b>均已作废</b>（初版确为 2~6 / 1.6s / 穿透 10%，此后被改；<b>伤害下限从来没到过 1</b>）。</para>
    /// <para>校准依据（param-calibration，dogcal）：设计定义以「高闪避+低伤」为核心对，「咬比爪快」为最弱 flavor 故让出
    /// ——咬合放缓到 2.3s（=爪击同速）、伤害下调，配 <c>Dog.DodgeChance</c>=0.45（见 <c>Dog.cs</c>）。
    /// <para>
    ...
    /// 此前这段内联抄着「胜率 39%（band 中位）、时长 33s、匕首基线 14.5s」，<b>四个数全部失效，且「band 中位」这个结论是反的</b>。
    /// </para>
    /// <para>
    /// ⚠ <b>已知偏离，待用户拍板</b>（2026-07-17 由 sweep-actors 重跑复现，与 committed 报告一致）：
    /// 布鲁斯 solo vs 丧尸胜率 <b>已高出「目标 30~45%」区间上限</b>（报告的判定行自己就在这么报）。
    /// 缠斗感本身仍成立（solo 时长 ≫ 匕首基线），2v1 稳赢≈100% 亦未变。
    /// <b>是否回调 0.45 / 还是重标目标区间＝数值决策，未自裁</b> —— 改这个数会动感知与 Sim 基线，须走 [DECISION]。
    /// </para></para>
    /// </summary>
    ...
    /// 也低于狗咬 <b>2~4</b> 的 3.0——赤手打人本就不该比一把刀更行）；
    /// ⚠️ 但<b>按 DPS 反过来</b>：拳脚 1.43 ＞ 狗咬 1.30（拳头冷却 1.4s，咬合 2.3s）——布鲁斯的战力不在咬合 DPS 上，
    /// 而在 <c>Dog.DodgeChance</c>=0.45 的高闪避缠斗（见 <see cref="DogBite"/>）。**别拿这两个数直接比强弱。**
    /// （旧注释写的"狗咬 1~4"已作废，真值 2~4。）；穿透 0（全表唯一——皮夹克都能让拳头变成挠痒痒，
