...
/// </para>
/// <para>
/// 为什么丧尸需要衣服：腐皮锐防/钝防均为 3。逐层结算里 atk ∈ [伤害下限,上限]、def ∈ [0, 护甲值×(1−穿透)]，
/// <c>atk &lt; def/2</c> 才算挡下 → <b>单层甲有可能挡下一击 ⟺ 护甲值×(1−穿透)/2 &gt; 武器伤害下限</b>。
/// 腐皮的门槛只有 1.5，低于任何武器的伤害下限 ⇒ 光靠腐皮对全部武器 <b>0% 阻挡（数学恒零）</b>。
...
/// （"衣服被砍坏了"落在**掉落**那一侧：尸体被搜刮时逐件掷骰，见消费层 <c>CorpseLoot</c>——
/// 打折损失的是战利品，不是它活着时的防护。）
/// <para>
/// <b>日常丧尸头/脚恒裸</b>：日常池里没有头盔也没有鞋（运动鞋是玩家开局装备）。
/// <b>但精英不再是</b>（[SPEC-B19]）：护甲表补进了人形头盔（军用头盔 / 防暴头盔），两套精英预设各戴一顶——
/// "爆头是精英的唯一软肋"这句话到此为止。理由是数字：`头` 为 Vital、MaxHp 16、命中权重 6（≈**5.8% 的命中落在头上**），
...
public static class ZombieOutfit
{
    ...
    /// <b>85% 的丧尸至少还穿着一件</b>；只有日常衣物（布衣/夹克/长裤/短裤/背心/外套），**没有任何护甲件**。
    /// </summary>
    public static IReadOnlyList<ZombieOutfitPreset> Presets { get; } = new ZombieOutfitPreset[]
    {
        ...
        // 外套层四件互斥（粗布外套 6/3 / 布夹克 8/4 / 牛仔外套 10/5 / 皮夹克 12/6），合计 20%，
    ...
    public static IReadOnlyList<ZombieOutfitPreset> ElitePresets { get; } = new ZombieOutfitPreset[]
    {
        new()
        {
            // 板甲（锐防 50/钝防 25，护躯干+双臂+双腿）= 全表最强 + 防暴头盔（护头 + 整张脸）。
