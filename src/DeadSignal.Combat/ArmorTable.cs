...
public static class ArmorTable
{
    ...
    // 头部是 Vital；具体 HP、命中权重与护甲数值均以 Wiki 配置表为准。
    ...
    //    · **军用头盔** = **泛用款**：防护、重量、覆盖与效果以 Wiki 配置表为准。轻、没有副作用，
    //      眼/面还空着（能再扣防毒面具）⇒ 什么场合都能戴，代价是脸完全裸露（挖眼、戳脸放血照旧成立）。
    //    · **防暴头盔** = **重装款**：防护全面更高、重量与覆盖以 Wiki 配置表为准，护整张脸，
    //      外加 Wiki 备注中的感知 debuff（钢壳闷在头上的代价）。
    //      ⚠️ 那两条 debuff 是**引擎新轴**（视野/听觉系数目前不吃穿戴品）⇒ **未落地、已挂起统一立项**；
    //         用户写的字保留在数值表的备注里，别当它已经生效。
    // ⇒ 取舍从"要脸还是要重量"升级成 **"要泛用的轻盔，还是要更硬但更笨重、还会削感知的重盔"**。
    // 面罩护脸换来的到底是什么：眼部命中只影响眼部后果——**它不是"防瞎"**，
    ...
    /// <summary>
    /// 军用头盔（头槽，<b>只护颅顶</b>）：**泛用款**，防护与重量以 Wiki 配置表为准；没有副作用，
    /// 且眼/面还空着 ⇒ 能再扣一张防毒面具。
    ...
    /// 代价是更高重量（吃负重上限）+ 占眼/面槽（与防毒面具互斥）+ Wiki 备注中的感知 debuff
    ...
    /// 但一只只占一只手槽、只护那一只手；护住双手要<b>两件</b>（[SPEC-B18-补]，重量按件计）。
    ...
    /// <para>
    /// 数值取用户填的，具体以 Wiki 配置表为准，与布类穿戴品同一条基线。
    /// 它补的是**头槽的布甲缺口**：在此之前头槽只有军用头盔与防暴头盔两件<b>重甲</b>，
    /// 开局根本够不着 ⇒ 头（本体最要命的部位之一）在整个前期完全裸着。一顶布帽子
    ...
    /// <summary>
    /// <b>恐怖装甲</b>（装甲层，护胸 + 腹）—— 数值见 Wiki 配置表。
    /// <para>
    /// 数值取用户填的。它定位是<b>「装甲层的廉价档」</b>：比皮革胸甲弱一档、重量较轻、且不吃皮革产能
    ...
    /// <summary>
    /// <b>墨镜</b>（眼镜槽，护双眼）—— 数值见 Wiki 配置表。
    /// <para>
    /// 🔴 <b>它的价值不只在基础防御，更在它占的槽</b>：眼镜槽上还坐着<b>防暴头盔 / 战争面具 / 防毒面具</b>三件
    /// （它们都要连着占面部槽）⇒ <b>戴墨镜 = 放弃这三件里的任何一件</b>。这是个真取舍，且是本表第一次出现
    /// 「只占眼镜槽、不占面部槽」的东西 —— 在它之前，眼镜槽从来没有过独立的候选人。
    /// </para>
    /// <para>
    /// ⚠️ 用户写的白天视野效果是<b>引擎新轴</b>（视野系数目前不吃穿戴品）⇒ <b>效果未做</b>，
    /// 已列入挂起的新轴清单统一立项。效果落地前，它就是一件"用一个宝贵槽位换基础护眼防护"的装备 ——
    ...
    /// <summary>
    /// <b>平光眼镜</b>（眼镜槽，护双眼）—— 数值见 Wiki 配置表，与<see cref="Sunglasses"/> 同槽互斥。
    /// <para>
    /// ✅ 用户写的阅读效果<b>已落地</b>：
    ...
    /// 从真实穿戴品名乘算汇总，由 <c>Pawn</c> 喂进 <c>ReadingSpeed.Effective</c> 的 apparelMult。
    ...
    /// <summary>
    /// [T71] <b>自制简易墨镜</b>（眼镜槽，护双眼）—— 数值见 Wiki 配置表。用户 authored（数值表『护甲表』new_armor_2）。
    /// <para>
    ...
    /// 价值就是 Wiki 配置表中的护眼防护，代价是同样要放弃眼镜槽上的头盔/面具。
    /// </para>
    /// </summary>
    public static ArmorLayer SelfMadeSnowGoggles() => Cfg("self_made_snow_goggles");

    /// <summary>
    /// [T72] <b>护踝鞋具</b>（成对·脚槽，护小腿+脚含趾）—— 数值见 Wiki 配置表。用户 authored（数值表『护甲表』new_armor_2）。
    /// <para>
    ...
    /// <summary>
    /// [警察局] <b>防弹背心</b>（<b>贴身层</b>·护胸 + 腹）—— 数值见 Wiki 配置表。用户 authored（新探索关「警察局」掉落）。
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Wiki 另有针对子弹的额外防护备注，后者仍是 Wiki 备注，未接入统一伤害类型规则。
    /// </para>
    /// <para>
    ...
    /// <summary>
    /// 口袋狗衣（身体·贴身层，护胸+腹）：[SPEC-B18] 起<b>不再是无甲纯容器</b>——护甲与容量均以 Wiki/消费层配置为准。
    /// </summary>
