...
/// <summary>
/// 守卫岗位类型（D 守卫防御战）。三种岗位各有一条主机制属性，当前数值以 Wiki 配置为准。
///  - 哨塔 <see cref="Watchtower"/>：远程射程/视野加成 + 围栏抵挡远程。实心障碍（走 AddSolid，会挖导航洞）。
///  - 屋顶平台 <see cref="RoofPlatform"/>：远程射程/视野加成（无抵挡）。实心障碍。
...
/// 由 <see cref="For"/> 按类型给出 Wiki 配置值；接入战斗见 <c>Actor.ApplyGuardPost</c>，
