...
public static class NightWatchContest
{
    ...
    public static readonly IReadOnlyDictionary<string, float> ApparelStealth = new Dictionary<string, float>
    {
        ...
        ["护踝鞋具"] = 0.00f,         // [T72] 高帮硬底：硬底踏地有声(负) ↔ 深色高帮遮住小腿反光(正)，两相抵近中性
        ...
        ["墨镜"] = -0.03f,            // 夜里戴墨镜＝把本就不多的月光又挡掉一层，看不清＝更容易踩响东西；镜面还可能反光 ⇒ 小负
        ...
        // ── 书籍解锁穿戴品（装备描述/重量来自 Wiki 护甲表；数值先给可调初值）──
        ["牛仔帽"] = -0.02f,         // 宽沿能遮轮廓，但皮面反光、摩擦有声，整体轻微负面
        ["马靴"] = -0.06f,           // 硬皮长筒 + 硬底，脚步明显；成对穿戴仍按一件物品贡献一次
        ["简易装甲"] = -0.18f,       // 5kg 金属片与厚布拼接，碰撞作响；潜行代价介于皮甲与板甲之间
    };
