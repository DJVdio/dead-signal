...
// 布鲁斯（狗）的饥饿刻度——**犬类最简**，与人类 HungerState 分开（进食节奏不同：吃一份 +3、每聚餐相位 -1）。

/// <summary>
/// 布鲁斯（狗）的饥饿刻度状态机（用户口径定稿）：吃一份食物 <see cref="EatGain"/>=+3；每聚餐相位无条件 -1；
...
{
    /// <summary>狗饥饿上限（拟定待调；吃一份 +3、每相位 -1，故一份约管三相位）。</summary>
    ...
    /// <summary>
    /// 一次聚餐相位结算（净模型，一次性施加，避免"-1 落 0 → +3 被短路"的跨 0 误杀）：
    /// 无条件 -1，吃到一份再 +<see cref="EatGain"/> —— 净变化 = 吃到 ? +2 : -1，clamp 到 [0, Cap]。
    /// 结算前已饿死者（终态）不复活、维持 0。返回结算后是否饿死。
    /// </summary>
    /// <param name="ate">本相位是否吃到一份食物（由营地层按余粮决定）。</param>
