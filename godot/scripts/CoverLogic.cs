...
///    偶尔钉在掩体上——概率读取 Wiki 配置；但**不做弹道与掩体的物理碰撞**（用户原话："这样不好做"）。</para>
/// </summary>
public readonly struct HalfCover
{
    ...
    ///
    /// <para>这条造出用户要的两难：一群丧尸贴着围栏在啃，你要么<b>开枪</b>（吃 Wiki 配置的掩体惩罚 + 烧子弹 + 枪声引怪），
...
/// 会有 Wiki 配置的无伤概率。逻辑上来说是敌人的子弹射在掩体上了，但是**这样不好做**，游戏的表现是
/// **这一下射中了人，但是按配置判定无效**。」</para>
///
...
///
/// <para><b>双向对称</b>：本类是纯几何函数、不认阵营。你朝躲在桌后的劫掠者开枪，一样按配置判定无效。</para>
/// </summary>
public static class CoverLogic
{
    /// <summary>半身掩体默认无伤几率；当前值以 Wiki 配置为准。</summary>
    ...
    /// 是**得走到那张桌子边上**。阈值由当前配置决定。站在房间中央声称自己有掩体不作数。</para>
    ///
    ...
    ///
    /// <para>用户拍板「<b>不允许隔着围栏近战</b>」。这条必须显式判——**光靠碰撞体挡不住**：
    /// 近战范围与围栏厚度可能让攻击者隔栏够到目标，故在出手前用同一套线段-矩形几何拦掉。</para>
    ///
