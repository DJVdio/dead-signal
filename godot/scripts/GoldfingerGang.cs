...
public static class GoldfingerGang
{
    ...
    /// <para>
    /// ⚠️ <b>[T63] 下面这组数字已按修复后的 Arena 重算过</b>——拍这块板时用的是<b>漏消费震荡/骨折</b>的旧 Arena
    /// （见 <c>07e9eb4</c>：钝器胜率被系统性低估约一半）。<b>复核结论：这一刀砍对了，板不用翻。</b>
    /// 同一口径（修复后 Arena）下的 A/B——
    /// </para>
    /// <list type="bullet">
    /// <item><b>原案（2 手枪）</b>：中期玩家（3 人同持<b>消防斧</b>）潜行清哨 <b>94.9%</b>，
    /// 但<b>全身而退只有 3%、平均留下 2.54 处永久残缺</b>、惨胜 91% ⇒ <b>赢了，全队残废</b>。那不叫可行。</item>
    /// <item><b>现案（撤掉两把手枪）</b>：<b>99.5% / 全身而退 11% / 1.96 处残缺</b>、惨胜 72%
    /// ⇒ 赢了，有人挂彩。<b>这才叫可行。</b>（与用户拍板时看到的 99.8% / 12% / 2.04 几乎一致——
    /// 潜行清哨这条路早已贴着天花板，Arena 的修复动不了它。）</item>
    /// <item><b>正面仍然很贵</b>：逐波推进 <b>60.9%</b>、平均<b>阵亡 1.35 / 3 人</b>、惨胜 98%——赔掉小半个队。
    /// ⚠️ 但要诚实：修复前这一格报的是 46.7% ⇒ <b>正面强攻比当初告诉用户的要好打一档</b>
    /// （钝器/斧类拿回了被吞掉的震荡收益）。若用户嫌正面太软，这里是唯一该动的旋钮。</item>
    /// <item>🔴 <b>红线守住了</b>：「惊动全据点」<b>仍然≈0%</b>（消防斧 0.4% / 长剑 0.3% / 棍棒 0.0%）。
    /// <b>枪一响还是死。</b>降难度<b>没有</b>把这关变成无脑清场，
    /// 噪音设计（弓/匕首叫醒 0 人、手枪 2 人、步枪 5 人）<b>一格没动</b>。</item>
    /// </list>
    /// 数在 <c>docs/research/2026-07-14-goldfinger-calibration.md</c>，harness = <c>src/DeadSignal.Sim/GoldfingerCalibration.cs</c>。
    /// </para>
    /// <para>
