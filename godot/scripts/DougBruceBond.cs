...
// 一方死亡后：等级**冻结**（由调用方停止累加共同存活天数实现；累积天数由调用方随档保存）+ 依赖伙伴的技能失效
...
/// <summary>
/// 道格 &amp; 布鲁斯羁绊纯逻辑（零 Godot 依赖）。全静态纯函数 + Wiki 配置入口；本类自身不持久状态
/// （等级由调用方喂的「共同存活天数」经 <see cref="EvaluateLevel"/> 现算；调用方保存该累积量，死亡冻结＝调用方停喂）。
/// </summary>
