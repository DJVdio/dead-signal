...
- 用途：尸潮瞭望与南逃峡谷两段正式世界演出背景。`HordeLookoutCinematic` 从双筒中央近景缓慢缩到 `horde-overview.png` 全局俯瞰；`EscapeCorridor` 在队伍抵达后冻结世界，以 4.8 秒把相机缩到 0.45，分别展示桥未落/桥已落的完整峡谷局面，再进入既有结局面板。
- 文件：`horde-overview.png`、`canyon-bridge-raised.png`、`canyon-bridge-lowered.png`；均为 1672×941 RGB PNG、16:9。坏结局图严格保留两名远端哨兵且桥未落；好结局图沿用相同峡谷构图，桥已落并有三名迎接者；近岸不绘制主角，由运行时真实角色层叠加。
- 来源：OpenAI 内置 image generation，2026-07-21 生成；项目原创资产。好结局峡谷使用坏结局峡谷作为编辑目标，只改变桥状态和远端人物。
- 尸潮俯瞰最终 prompt：

  > Use case: stylized-concept. Asset type: production 16:9 cinematic environment background for a faux-isometric pixel-art zombie survival game. Primary request: a vast zombie horde seen from a camera that has risen high above a ruined city overlook, revealing the overwhelming full scale of the migration. Scene/backdrop: bleak northern plain and broken suburban edge stretching to a cold horizon; roads and fields are almost completely covered by an immense moving mass of ordinary human zombies. Subject: the horde itself as a terrifying broad dark flow, dense toward the horizon and readable as many individual ordinary zombies in the nearer portion; no special or elite zombies. Style/medium: crisp gritty hand-authored-looking pixel art, restrained realistic proportions, faux-isometric/high oblique aerial perspective, limited low-saturation survival-game palette. Composition/framing: wide 16:9 establishing overview; central visual flow leads from the distant horizon toward the lower foreground; enough environmental landmarks to communicate enormous scale; no binocular frame or UI because the game adds that at runtime. Lighting/mood: cold overcast late afternoon, oppressive haze, bleak and quiet, no dramatic fantasy lighting. Constraints: environment background only; no named characters, no close-up hero, no military forces, no vehicles in motion, no text, labels, logos, interface, frame, border, watermark, fireball, gore close-up or extra narrative event.
- 桥未落峡谷最终 prompt：

  > Use case: stylized-concept. Asset type: production 16:9 world-space cinematic background for the southern escape canyon endpoint in a faux-isometric pixel-art zombie survival game. Primary request: a bleak forest escape corridor ending at a deep canyon where the bridge from the far-side settlement has NOT been lowered. Scene/backdrop: narrow muddy path emerges from dense dark forest onto the near canyon rim; a broad impassable ravine cuts across the scene; the fortified far rim has a raised wooden-and-steel drawbridge that remains vertical and cannot be crossed. Subject: exactly two distant anonymous sentries stand on the far bridgehead watching coldly; no one stands on the near side because runtime characters will be drawn there by the game. Style/medium: crisp gritty hand-authored-looking pixel art, faux-isometric/high oblique aerial view, realistic environment proportions, restrained low-saturation survival-game palette. Composition/framing: wide 16:9 establishing view designed for a camera pull-up; near-side path enters from lower center, canyon runs across the middle, far settlement and raised bridge occupy the upper middle; generous empty near-side ground for runtime characters; readable bridge state at both close and wide framing. Lighting/mood: cold gray dawn, muted mist in the canyon, exhausted and hopeless, no theatrical fantasy lighting. Constraints: exactly two far-side sentries; bridge clearly raised and disconnected from the near rim; no protagonist, survivor group, zombies, military insignia, flags, text, dialogue, UI, labels, logos, border, watermark, gore or extra narrative event.
- 桥落下峡谷编辑 prompt：

  > Use case: precise-object-edit. Asset type: production alternate ending background for the same southern escape canyon scene. Input images: Image 1 is the exact bad-ending canyon composition and must remain the spatial/layout reference. Primary request: change only the ending state. Lower the existing far-side drawbridge so it spans the canyon and visibly connects the far settlement to the near-side path. Replace the two cold sentries with exactly three small anonymous welcoming people standing at the far bridgehead in practical survivor clothing, with restrained open postures. Constraints: preserve the exact 16:9 framing, camera angle, canyon geography, near-side empty path, forest, cliffs, settlement, palette, pixel-art rendering, lighting and all other environmental details from Image 1. Keep the near side empty for runtime player characters. The bridge must be clearly down and traversable. No named characters, protagonist group, zombies, military insignia, flags, text, UI, logos, watermark or additional objects.
...
- 后处理：`#ff00ff` 色键去底，soft matte + despill。模型两次尝试仍只输出 7 行，因此保留原 7 行全部像素与动作，以西南方向水平镜像补成东南行，再无缩放重排为 8 个等高方向行；随后将东南行的 12 个动作格整理回与其余方向一致的列顺序。全部验证为 1536×1176 RGBA，所有行均为第一列站立、最后一列卧姿，不需要运行时特判。
- prompt 摘要：每名角色独立生成、现实人体比例、保持 authored 外貌与服装色；步行必须换腿摆臂，攻击/工作重画躯干手臂，坐卧必须重画而非旋转站立帧；空手、无道具、无文字。布鲁斯与泛用犬改用对应犬类动作列。
- 东南补帧编辑 prompt 摘要：严格保留既有角色身份、服装、比例、调色板、动作列与前 7 个方向，补第 8 行东南动作并保持 12×8 精确网格；因模型仍返回 7 行，最终采用上述确定性镜像与标准列重排方案。

## attacks/*.png

- 用途：7 名具名幸存者及泛用幸存者/劫掠者的持械身体动作，共 9 张；运行时依据这一次真正打出的武器选择姿势，真实武器仍由 paper-doll 装备层绘制。
- 规格：每张 2688×1176 RGBA，精确 21×8 网格，单格 128×147；每类动作连续三列为蓄力、出手、收招，7 类依次为单手挥砍、单手戳刺、单手射击、双手挥砍、双手戳刺、双手射击、双手拉弓射箭，8 行依次为南、西南、西、西北、北、东北、东、东南。
- 来源：OpenAI 内置 image generation，2026-07-21 以既有角色身份和服装为参考生成；项目原创资产。生成模式为内置图像生成。
- 后处理：`#ff00ff` 色键经官方 chroma helper 以 soft matte + despill 去底；保留模型稳定输出的南/西南/西/西北/北五个原生方向，东北/东/东南分别由西北/西/西南确定性水平镜像补齐，再逐格最近邻整理到标准尺寸。三帧序列由 `tools/build_attack_animation_atlases.py` 确定性组合：既有空手攻击帧提供蓄力/收招，七类专属帧提供出手，并清除与主体断开的生成碎片；脚本可重复运行。密集 14×8 首稿、大头诺蒂首稿及会改变身份的无武器重绘稿均未采用。
- 最终 prompt 组摘要：每名角色独立生成，保持正式图集中的 authored 外貌、现实人体比例、服装与脚点；精确七列，逐列要求单手横挥、单手直刺、单手瞄准后坐、双手过肩挥砍、双手长柄直刺、双手抵肩射击、双手拉弓释放；空手、透明处理用纯洋红背景，无文字、标签、边框、网格或额外人物。泛用幸存者/劫掠者使用同一动作结构与各自正式身份色板。运行时真实持械层覆盖动作参考中的手部占位物，不把生成图里的武器当装备状态。
